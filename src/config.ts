...
export function detectBestMethod(): Exclude<NotificationMethod, "auto"> {
  ...
  if (termProgram === "kitty" || termProgram === "wezterm" || termProgram === "foot") {
    return "osc99"
  }
...
function isValidMethod(method: unknown): method is NotificationMethod {
  return typeof method === "string" && ["bell", "osc9", "osc777", "osc99", "auto"].includes(method)
}

function parseEventConfig(
  userEvent: boolean | { enabled?: boolean; method?: NotificationMethod } | undefined,
...
export function loadConfig(): NotifierConfig {
  ...
  try {
    ...
    const globalMethod = isValidMethod(userConfig.method) ? userConfig.method : DEFAULT_CONFIG.method
    ...
    return {
      ...
      events: {
        ...
        subagent_complete: parseEventConfig(userConfig.events?.subagent_complete, { enabled: false }),
...
export function getEventMethod(config: NotifierConfig, event: EventType): Exclude<NotificationMethod, "auto"> {
